# По аналогии приведенного кода python калькулятора можно сдель, допустим, календарь.
# Для этого надо будет запрашивать текущую дату у операционной системы.
# Открывать нужный месяц, рассчитывать какие числа выпадут на понедельники, какой год високосный.
# Сделать возможность менять год и месяцы.

# Использую стандартные библиотеки tkinter, datetime и calendar
# Для определения текущего дня недели и количества дней в месяце использовалась стандартная библиотека calendar.
from tkinter import *
import calendar
import datetime
root = Tk()
...
def prew():
    ...
    if month == 0:
        month = 12
        year -= 1
    fill()  # функция перерисовывает календарь
...
def fill():
    ...
    if month == 1:
        prew_month_days = calendar.monthrange(year-1, 12)[1]    # Вычисляем количество дней в предыдущем месяце
    else:
...
for n in range(7):
    lbl = Label(root, text=calendar.day_abbr[n], width=1, height=1,
                font=('Verdana', 10, 'normal'), fg='darkred')
